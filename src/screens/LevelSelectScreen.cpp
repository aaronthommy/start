...
void LevelSelectScreen::updateCardAnimations(float deltaTime)
{
    for (LevelCard &card : levelCards)
    {
        if (card.isHovered)
        {
            ...
            if (card.hoverScale > 1.1f)
                card.hoverScale = 1.1f;
        ...
        else
        {
            ...
            if (card.hoverScale < 1.0f)
                card.hoverScale = 1.0f;
...
void LevelSelectScreen::drawCardInfo(const LevelCard &card) const
{
    ...
    Color textColor = card.status == LevelStatus::LOCKED ? Color{120, 120, 120, 255} : Color{50, 50, 50, 255};
