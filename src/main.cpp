...
int main()
{
    ...
    mainMenu.load("", {{"Spiel starten", [&]
                       ...
                       {"Progress Reset", [&] { // <-- NEU
                            ...
                            saveManager.reset();
                            TraceLog(LOG_INFO, "Save progress reset from menu!");
                        }},
                       {"Zurück", [&]
    ...
    gameScreen.setOnFinish([&]
                           {
                               gameScreen.unload();
                               levelSelect.refresh();           // Refresh Level-Status nach dem Spiel
                               state = GameState::LEVEL_SELECT; // Zurück zur Level-Auswahl statt Main Menu
                           });
